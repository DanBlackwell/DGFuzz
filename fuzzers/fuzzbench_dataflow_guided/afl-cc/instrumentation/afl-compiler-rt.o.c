...
static u8 *__afl_dataflow_map_dummy = __afl_dataflow_initial;
static u8 *__afl_dataflow_map_backup = __afl_dataflow_initial;
...
static void __afl_map_shm(void) {
  ...
  if (id_str) {
    ...
    __afl_dataflow_ptr = shm_base + __afl_map_size;
#else
    u32 shm_id = atoi(id_str);

    if (__afl_map_size && __afl_map_size > 2 * MAP_SIZE) {
    ...
    __afl_dataflow_map_dummy = __afl_area_ptr_dummy + __afl_final_loc;
    __afl_dataflow_ptr = __afl_dataflow_map_dummy;
  ...
  __afl_dataflow_map_backup = __afl_dataflow_ptr;
  ...
  if (__afl_debug) {
    ...
    fprintf(stderr,
            ...
            id_str == NULL ? "<null>" : id_str, __afl_area_ptr,
            __afl_area_initial, __afl_dataflow_ptr, __afl_area_ptr_dummy, __afl_map_addr, 
            MAP_SIZE, __afl_final_loc, __afl_map_size);
  ...
  if (__afl_selective_coverage) {
    ...
    if (__afl_map_size > MAP_INITIAL_SIZE) {
      ...
      __afl_dataflow_map_dummy = __afl_area_ptr_dummy + __afl_map_size;
...
__attribute__((constructor(1))) void __afl_auto_second(void) {
  ...
  if (__afl_final_loc > MAP_INITIAL_SIZE) {
    ...
    if (__afl_map_addr)
      ...
      ptr = (u8 *)malloc(2 * __afl_first_final_loc);
    ...
    if (ptr && (ssize_t)ptr != -1) {
      ...
      __afl_area_ptr_backup = __afl_area_ptr;
      __afl_dataflow_ptr = ptr + __afl_map_size;
      __afl_dataflow_map_dummy = __afl_dataflow_ptr;
      __afl_dataflow_map_backup = __afl_dataflow_ptr;
